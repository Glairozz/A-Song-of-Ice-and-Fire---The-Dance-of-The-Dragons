window.addEventListener("load", () => {
    ...
    function initPremiumScrollReveal() {
        ...
        document.querySelectorAll('.scroll-reveal, .scroll-reveal-left, .scroll-reveal-right, .scroll-reveal-scale, .scroll-reveal-3d').forEach(el => {
            observer.observe(el);
        });

        // Observe cards and other elements
        document.querySelectorAll('.compact-card, .character, .org, .races, .locations, .magic, .dragon-container, .house').forEach((el, index) => {
            el.style.opacity = '0';
        ...
        style.textContent = `
            .compact-card.visible, .character.visible, .org.visible, .races.visible, .locations.visible, .magic.visible, .dragon-container.visible, .house.visible {
                opacity: 1 !important;
    ...
    function initMagneticButtons() {
        const buttons = document.querySelectorAll('.view-info-btn, .btn-premium, .return-to-top');
    ...
    function initRippleEffect() {
        document.querySelectorAll('.compact-card, .character, .org, .races').forEach(card => {
    ...
    function initTiltEffect() {
        const cards = document.querySelectorAll('.compact-card, .character, .org, .races');
    ...
    const infoModalData = {
        characters: {
            ...
            velaryon: [
                ...
                { name: "Vaemond Velaryon", title: "Lord of Driftmark", image: "characters/velaryon/vaemond.webp", description: "Vaemond Velaryon challenged the legitimacy of Rhaenyra's sons. He was killed by Daemon Targaryen.", era: "House of the Dragon Era" }
            ],
            hightower: [
    ...
    // Race data
    const raceData = [
        { name: "First Men", description: "The oldest human race in Westeros; strong ties to the Old Gods. Northerners like the Starks descend from them.", image: "races/firstmen.jpg", origin: "Westeros" },
        { name: "Andals", description: "Invaders who conquered most of Westeros after the First Men; followers of the Faith of the Seven.", image: "races/andals.jpg", origin: "Essos (Andal Invasion)" },
        { name: "Valyrians", description: "Dragonlords from ancient Valyria with silver-gold hair and violet eyes; ancestors of the Targaryens.", image: "races/valyrians.jpg", origin: "Valyria" },
        { name: "Children of the Forest", description: "Small, magical, ancient beings who lived in Westeros before humans; created the weirwoods.", image: "races/children.jpg", origin: "Westeros (Ancient)" },
        { name: "Giants", description: "Large, near-extinct humanoids beyond the Wall; primitive but intelligent.", image: "races/giants.jpg", origin: "Beyond the Wall" },
        { name: "White Walkers", description: "Supernatural ice beings who command the dead; ancient enemies of the living.", image: "races/whitewalkers.jpg", origin: "Land of Always Winter" },
        { name: "Wights", description: "Reanimated corpses controlled by the White Walkers; not a race, but an undead force.", image: "races/wights.jpg", origin: "White Walkers" },
        { name: "Free Folk", description: "Human tribes north of the Wall; fiercely independent and culturally unique.", image: "races/wildlings.jpg", origin: "Beyond the Wall" },
        { name: "Essosi Peoples", description: "Includes Dothraki, Ghiscari, Qartheen, Braavosi, and more.", image: "races/essosi.jpg", origin: "Essos" }
    ];

    // Location data
    const locationData = [
        { name: "King's Landing", description: "Capital of the Seven Kingdoms; home to the Red Keep and the Iron Throne.", image: "location/kingslanding.webp", region: "Crownlands" },
        { name: "Dragonstone", description: "Volcanic island and ancestral Targaryen seat; major dragon-hatching site.", image: "location/dragonstone.jpg", region: "Crownlands" },
        { name: "Winterfell", description: "Ancient northern stronghold of House Stark; built atop hot springs.", image: "location/winterfell.jpg", region: "The North" },
        { name: "The Wall", description: "A colossal frozen barrier defending the realms of men from the far North.", image: "location/thewall.jpg", region: "The North" },
        { name: "Beyond the Wall", description: "Harsh wilderness inhabited by Free Folk, giants, and White Walkers.", image: "location/beyondthewall.jpg", region: "Far North" },
        { name: "Oldtown", description: "Seat of the Hightowers; home to the Citadel and the Faith's origins.", image: "location/oldtown.webp", region: "The Reach" },
        { name: "The Riverlands", description: "A central region often caught in wars; includes Riverrun and Harrenhal.", image: "location/riverlands.jpg", region: "Riverlands" },
        { name: "The Vale of Arryn", description: "Mountainous, isolated region guarded by the Eyrie.", image: "location/vale.webp", region: "The Vale" },
        { name: "The Iron Islands", description: "Harsh seafaring land ruled by House Greyjoy.", image: "location/ironislands.jpg", region: "Iron Islands" },
        { name: "Dorne", description: "Southern desert kingdom known for a hot climate and different customs.", image: "location/dorne.avif", region: "Dorne" }
    ];

    // Organization data
    const orgData = [
        { name: "Night's Watch", description: "A sworn brotherhood protecting the Wall; vows of celibacy, loyalty, and service.", image: "org/nightwatch.jpg", type: "Military Order" },
        { name: "Kingsguard", description: "Elite white-armored knights who protect the king.", image: "org/kingsguard.jpg", type: "Royal Guard" },
        { name: "Small Council", description: "Ruling advisory council in King's Landing; handles royal governance.", image: "org/smallcouncil.webp", type: "Government" },
        { name: "The Citadel", description: "Scholarly order in Oldtown where maesters train.", image: "org/citadel.webp", type: "Educational" },
        { name: "Faith of the Seven", description: "Main religion of most Westerosi; includes septons, septas, and the Faith Militant.", image: "org/faith7.webp", type: "Religion" },
        { name: "Dothraki Khalsar", description: "Nomadic horse tribe led by a khal; structured around warfare and raiding.", image: "org/khalsar.webp", type: "Tribe" },
        { name: "Unsullied", description: "Elite slave-soldier army trained for discipline and obedience.", image: "org/unsullied.jpg", type: "Mercenary Army" },
        { name: "Iron Bank of Braavos", description: "The most powerful financial institution in the known world.", image: "org/ironbank.jpg", type: "Bank" },
        { name: "Faceless Men", description: "An ancient assassin cult from Braavos who worship the Many-Faced God.", image: "org/facelessmen.jpg", type: "Assassin Cult" },
        { name: "Golden Company", description: "A massive, elite sellsword army founded by exiled Westerosi.", image: "org/goldencompany.jpg", type: "Mercenary Army" }
    ];

    // Magic data
    const magicData = [
        { name: "Dragons", description: "Fire-breathing reptiles born of Valyria; magic returns to the world when Daenerys hatches three eggs.", image: "magic/dragons/d1.jpg", type: "Creatures" },
        { name: "Warging / Skinchanging", description: "Ability to enter the minds of animals or other beings; strongest among the First Men and wildlings.", image: "magic/skinchanging.jpg", type: "Magic Power" },
        { name: "Greensight", description: "Prophetic visions or dreams; sometimes linked to weirwoods or children of the forest.", image: "magic/greensight.jpg", type: "Magic Power" },
        { name: "Weirwood Trees", description: "White-barked, red-leafed trees tied to ancient magic and memory.", image: "magic/weirwoodtrees.webp", type: "Magical Objects" },
        { name: "Ice Magic", description: "Magic that raises the dead, controls cold, and empowers ice creatures.", image: "magic/icemagic.jpg", type: "Magic Power" },
        { name: "Fire Magic", description: "Miracles like resurrection (Beric, Jon Snow), shadowborn assassins, and prophetic flames.", image: "magic/firemagic.jpg", type: "Magic Power" },
        { name: "Valyrian Steel", description: "Magic-forged metal that can kill White Walkers; nearly lost art.", image: "magic/valyriansteel.jpg", type: "Weapons" },
        { name: "Glass Candles", description: "Obsidian artifacts used by the Valyrians for seeing across distances and influencing dreams.", image: "magic/glasscandles.jpg", type: "Magical Objects" },
        { name: "Dragon glass", description: "Volcanic glass that kills White Walkers; found in abundance at Dragonstone.", image: "magic/dragonglass.jpg", type: "Weapons" }
    ];

    // Create compact cards function
    function createCompactCards(data, container, cardClass, subtitleClass, modalType) {
        if (!container) return;
        ...
        data.forEach((item, index) => {
            ...
            card.innerHTML = `
                ...
                <button class="view-info-btn" data-item-index="${index}" data-modal-type="${modalType}">View Information</button>
            `;
    ...
    function showInfoModal(type, index) {
        ...
        switch(type) {
            case 'dragons': item = dragonData[index]; break;
            case 'races': item = raceData[index]; break;
            case 'locations': item = locationData[index]; break;
            case 'organizations': item = orgData[index]; break;
            case 'magic': item = magicData[index]; break;
            default: return;
        }
    ...
    if (dragonCardsContainer) {
        createCompactCards(dragonData, dragonCardsContainer, 'compact-card dragon-compact', 'house-origin', 'dragons');
    }

    const raceCardsContainer = document.getElementById('raceCardsContainer');
    if (raceCardsContainer) {
        createCompactCards(raceData, raceCardsContainer, 'compact-card race-compact', 'house-origin', 'races');
    }

    const locationCardsContainer = document.getElementById('locationCardsContainer');
    if (locationCardsContainer) {
        createCompactCards(locationData, locationCardsContainer, 'compact-card location-compact', 'house-origin', 'locations');
    }

    const orgCardsContainer = document.getElementById('orgCardsContainer');
    if (orgCardsContainer) {
        createCompactCards(orgData, orgCardsContainer, 'compact-card org-compact', 'house-origin', 'organizations');
    }

    const magicCardsContainer = document.getElementById('magicCardsContainer');
    if (magicCardsContainer) {
        createCompactCards(magicData, magicCardsContainer, 'compact-card magic-compact', 'house-origin', 'magic');
    }
    ...
    const characterHouses = ['targaryan', 'stark', 'lannister', 'velaryon', 'hightower', 'baratheon', 'tyrell', 'tully', 'arryn', 'mormont', 'frey', 'martell'];
    characterHouses.forEach(house => {
        initCharacterCards(house, house + 'Chars');
    });
});